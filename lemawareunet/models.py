...
class UnetBottleneckAssisted(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob):
        ...
        for i in range(num_pool_layers - 1):
            self.down_sample_layers += [ConvBlock(ch, ch * 2, drop_prob)]
            ch *= 2
        self.conv = nn.ModuleList([ConvBlock(ch, ch, drop_prob), nn.Conv2d(ch*3,ch,kernel_size=1)])
    ...
    def forward(self, input,feat):
        ...
        for layer in self.down_sample_layers:
            ...
            stack.append(output)
            output = F.max_pool2d(output, kernel_size=2)
        _,output = self.conv[0](output)
        latentcat = torch.cat([output,feat],dim=1)
        output = self.conv[1](latentcat)
