...
from models import DnCn,UnetModel,DnCnFeature,UnetBottleneckAssisted
import torchvision
...
from unetmodels import UnetModelAssistEverywhere
...
def create_data_loaders(args):
    ...
    display_data = [dev_data[i] for i in range(0, len(dev_data), len(dev_data) // 4)]
...
def train_epoch(args, epoch, unetmodel, dncnmodel, model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        if args.dataset_type == 'cardiac':
            ...
            input = F.pad(input,(5,5,5,5),"constant",0)
            target = F.pad(target,(5,5,5,5),"constant",0)

        feat,seg = unetmodel(rec)
        #if args.dataset_type == 'cardiac':
        #    feat = feat[:,:,5:155,5:155]
        #print("input and feat shape: ",input.shape,feat.shape)
...
def evaluate(args, epoch, unetmodel, dncnmodel, model, data_loader, writer):
...
def visualize(args, epoch, unetmodel, dncnmodel, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            #output = model(input.float(), latent)
            output = model(input.float(), feat)
...
def build_model(args):
    ...
    model = UnetModelAssistEverywhere(
         in_chans=1,
         out_chans=1,
         chans=args.num_chans,
         num_pool_layers=args.num_pools,
...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_loss,train_time = train_epoch(args, epoch, segmodel, recmodel, model, train_loader,optimizer,writer)
        dev_loss,dev_time = evaluate(args, epoch, segmodel, recmodel, model, dev_loader, writer)
        visualize(args, epoch, segmodel, recmodel, model, display_loader, writer)
