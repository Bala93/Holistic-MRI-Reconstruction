...
def create_datasets(args):
    ...
    train_data = SliceDataMrbrain(args.train_path,args.acceleration)
    dev_data = SliceDataEvaluateMrbrain(args.val_path,args.acceleration, args.dataset_type,args.dncn_model_path)
...
def train_epoch(args, epoch, unetmodel, model, netD, data_loader, optimizer, optimizerD, writer, us_mask):
    model.train()
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        input, kspace, t1imgfs, target = data
        ...
        outG = model(input, kspace, feat, us_mask1, seg, t1imgfs)
        #print (outG.shape)
...
def evaluate(args, epoch, unetmodel, model, data_loader, writer, us_mask):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            input, kspace, target, predictedimg, t1imgfs = data
            ...
            output = model(input,kspace,feat,us_mask1,seg,t1imgfs)
...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_lossG, train_lossD, train_time = train_epoch(args, epoch, segmodel, model, disc, train_loader, optimizer, optimizerD, writer,us_mask)
        dev_psnr, dev_ssim, dev_vif, dev_time = evaluate(args, epoch, segmodel,model, dev_loader, writer,us_mask)
