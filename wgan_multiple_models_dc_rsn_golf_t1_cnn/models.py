...
class UnetModelTakeEverywhereWithIntermediate(nn.Module):
    """
    PyTorch implementation of a U-Net model.
    This is based on:
        Olaf Ronneberger, Philipp Fischer, and Thomas Brox. U-net: Convolutional networks
...
class ReconSynergyNetAblative(nn.Module):
    def __init__(self, args,**kwargs):
        ...
        if args.dataset_type=='cardiac':
            patch_size = 150
        elif args.dataset_type=='mrbrain_t1' or args.dataset_type == 'mrbrain_flair':
            patch_size = 240
...
class ReconSynergyNetAblativeFeature(nn.Module):
    def __init__(self, args,**kwargs):
        ...
        unet_model = UnetModelAssistEverywhere(1,1,32,4,0) #working *******
...
class Conv1x1feat(nn.Module):
    def __init__(self,n_in=512,n_out=32):

        super(Conv1x1feat, self).__init__()
        #self.conv1x1=nn.Conv2d(n_in,n_out,kernel_size=1)
        self.conv1x1=nn.Sequential(nn.Conv2d(n_in,n_out,kernel_size=1),nn.ReLU())
        #self.conv1=nn.Sequential(nn.Conv2d(n_out,n_out,kernel_size=3,padding=1),nn.ReLU())
        #self.conv1x1down=nn.Sequential(nn.Conv2d(480,n_out,kernel_size=1),nn.ReLU())
        #self.conv1x1up=nn.Sequential(nn.Conv2d(256,n_out,kernel_size=1),nn.ReLU())
        #self.conv1x1_2=nn.Sequential(nn.Conv2d(n_out,n_out,kernel_size=1),nn.ReLU())

    def forward(self,x):#,downfeat,upfeat):
        #x=self.conv1x1(x)
        #down = self.conv1x1down(downfeat)
        #up = self.conv1x1up(upfeat)
        return self.conv1x1(x)#, down, up
        #return self.conv1(x)#, down, up
...
class DnCnFeatureLoop(nn.Module):
    ...
    def __init__(self,args,n_channels=2, nc=5, nd=5,**kwargs):
        ...
        for i in range(nc): # actually we are only using 3 blocks , remaining 2 are not useed. 
