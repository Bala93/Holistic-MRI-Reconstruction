...
from models import DenseBlock,DnCn
import torchvision
...
def train_epoch(args, epoch, model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        output = model(input,input_kspace) #using dncn block sending input-kspace for correction
        #output = model(input,5) 
        #output = output + input # learn residue 
...
def evaluate(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            output = model(input,input_kspace) #dncn for dc
            #output = model(input,5)
            #output = output + input # learn residue 
            #output = F.pad(output,(5,5,5,5),'constant',0)
...
def visualize(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            input_kspace = input_kspace.to(args.device)
            target = target.unsqueeze(1).to(args.device)
            output = model(input.float(),input_kspace) #dncn for dc 
            #input = input.float()
            #output = model(input,5)
            #output = output + input # learn residue 
            #output = F.pad(output,(5,5,5,5),'constant',0)
...
def build_model(args):
    ...
    model = DnCn(args,n_channels=1).to(args.device) # kirby 
    #model = DenseBlock(n_ch=1).to(args.device) # using for cardiac 
    #print (model)
