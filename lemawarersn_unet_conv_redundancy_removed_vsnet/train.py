...
from dataset import KneeData,KneeDataEvaluate
...
def create_datasets(args):
    ...
    dev_data = KneeDataEvaluate(args.validation_path,args.dncn_model_path)
...
def train_epoch(args, epoch, unetmodel, model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        rec = T.complex_abs(img_gt).unsqueeze(1)
...
def evaluate(args, epoch, unetmodel, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            #img_gt,img_und,img_und_kspace,rawdata_und,masks,sensitivity = data
            img_gt,img_und,img_und_kspace,rawdata_und,masks,sensitivity,predictedimg = data
...
def visualize(args, epoch, unetmodel, model, data_loader, writer):
...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_loss,train_time = train_epoch(args, epoch, segmodel, model, train_loader,optimizer,writer)
        dev_loss,dev_time = evaluate(args, epoch, segmodel, model, dev_loader, writer)
        visualize(args, epoch, segmodel, model, display_loader, writer)
