...
from models import DnCn,UnetModel,DnCnFeature,DnCnFeatureLoop,UnetModelTakeLatentDecoder
import h5py
...
def run_unet(args, segmodel, model, data_loader):
    ...
    with torch.no_grad():
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            recons = model(input, input_kspace,feat,us_mask1).to('cpu').squeeze(1)
