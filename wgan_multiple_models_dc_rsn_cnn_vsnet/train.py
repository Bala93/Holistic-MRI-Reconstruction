...
from dataset import KneeData, KneeDataDev
...
def create_datasets(args):
    ...
    dev_data = KneeData(args.val_path)
...
def train_epoch(args, epoch, model, netD, data_loader, optimizer, optimizerD, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        outG = model(img_und,img_und_kspace,rawdata_und,masks,sensitivity)
        outG = outG.permute(0,3,1,2)
...
def evaluate(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            output = T.complex_abs(output).cpu().numpy()[0,:,:]
            target = T.complex_abs(target).cpu().numpy()[0,:,:]
...
def visualize(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            print (output.shape, target.shape, img_und.shape)
...
def build_generator(args):
    ...
    model = networkRefine(args, mode='train').to(args.device)
    return model
