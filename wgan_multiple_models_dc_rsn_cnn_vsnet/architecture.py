...
from model_vsnet import ReconSynergyNetAblative
...
class cnn_layer_refine(nn.Module):
    
    def __init__(self):
        super(cnn_layer_refine, self).__init__()
...
class network(nn.Module):
    ...
    def __init__(self, alfa=1, beta=1, cascades=3):
...
class networkRefine(nn.Module):
    
    def __init__(self, args, alfa=1, beta=1, cascades=3, mode='train'):
        super(networkRefine, self).__init__()
        ...
        self.conv_block = cnn_layer_refine()
        self.dc_block = dataConsistencyTerm(alfa)
        self.wa_block = weightedAverageTerm(beta)
        
 
    def forward(self, x, k1, k, m, c):

        x = self.network(x, k1, k, m, c)        
        x_cnn = self.conv_block(x)
        Sx, SS = self.dc_block.perform(x, k, m, c)
        x = self.wa_block.perform(x_cnn, Sx, SS)
