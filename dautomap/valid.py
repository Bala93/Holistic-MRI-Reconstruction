...
def load_model(checkpoint_file):
    ...
    #size_lookup = {'cardiac':150,'mrbrain':240,'knee':320,'kirby90':256}
    #patch_size = size_lookup[args.dataset_type]
    patch_size = 256
...
def run_model(args, model, data_loader):
    ...
    with torch.no_grad():
        ...
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            if args.dataset_type == 'knee' :
                _, input, target,fnames = data
            else:
                _, input, target,fnames,slices = data

            #input = input.unsqueeze(1).to(args.device)
            input  = input.to(args.device)
            input = input.permute(0,3,1,2)
            recons = model(input.float()).to('cpu').squeeze(1)
