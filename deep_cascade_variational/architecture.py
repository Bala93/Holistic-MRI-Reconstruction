...
class dataConsistencyTerm(nn.Module):
    ...
    def perform(self, x, k0, mask, sensitivity):
        ...
        k = torch.fft(x, 2, normalized=True) * mask 
        A_x_y = k - k0
              
        x = torch.ifft(A_x_y, 2, normalized=True)
...
class network(nn.Module):
    ...
    def forward(self, x, k, m, c):
        ...
        for i in range(self.cascades):
            ...
            gradD_x = self.dc_blocks[i].perform(x, k, m, c)
            x = self.wa_blocks[i].perform(x + x_cnn,gradD_x)
        return x    
