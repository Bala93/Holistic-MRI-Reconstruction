...
def load_model(args,checkpoint_dccnn,checkpoint_percept,alpha):
    ...
    for param_name in checkpoint_dccnn['model']:
        #re_present =  ['Re' in key for key in checkpoint_dccnn['model'].keys()
        param_dccnn = checkpoint_dccnn['model'][param_name]
        param_percept = checkpoint_percept['modelG_dc'][param_name]
        if not 'Re' in param_name:
            param_combined[param_name] = param_dccnn
        else:
...
def run_gan(args, models_combined,data_loader):
    ...
    with torch.no_grad():
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            input_kspace = input_kspace.permute(0,3,1,2).to(args.device).float()
            input = input.float()
...
def main(args):
    ...
    all_alphas = list(np.linspace(0,1,6))
