SAVE_MODEL='adversarial_perceptual_reconsyn_gan'
MODEL='deep_cascade_recon_synergy_net'
MODEL_1='adversarial_perceptual_reconsyn_gan'
...
DEVICE='cuda:0'
DATA_PATH=${BASE_PATH}'/datasets/'${DATASET_TYPE}'/validation/acc_'${ACC_FACTOR}
echo python valid_interp_all_mod.py --checkpoint_dccnn ${CHECKPOINT} --checkpoint_percept ${CHECKPOINT_1}  --out-dir ${OUT_DIR} --batch-size ${BATCH_SIZE} --device ${DEVICE} --data-path ${DATA_PATH} --acceleration_factor ${ACC_FACTOR} --usmask_path ${USMASK_PATH} --dataset_type ${DATASET_TYPE} 
python valid_interp_all_mod.py --checkpoint_dccnn ${CHECKPOINT} --checkpoint_percept ${CHECKPOINT_1}  --out-dir ${OUT_DIR} --batch-size ${BATCH_SIZE} --device ${DEVICE} --data-path ${DATA_PATH} --acceleration_factor ${ACC_FACTOR} --usmask_path ${USMASK_PATH} --dataset_type ${DATASET_TYPE} 
#ACC_FACTOR_4x
