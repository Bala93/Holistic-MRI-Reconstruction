...
import argparse
from dataset_1 import SliceData
from models import DnCn,Discriminator,Vgg16
...
def train_epoch(args, epoch, modelG_dc,modelD, data_loader, optimizerG, optimizerD, writer, display_loader, exp_dir, vgg):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        outG = modelG_dc(input,input_kspace)
      
        ## padding is done in this stage to accomodate unet 160x160. ##
        ...
        lossG = img_scale * lossG_img #+ vgg_scale * lossG_vgg
...
def evaluate(args, epoch, model_dc, modelD, vgg, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            output = model_dc(input,input_kspace)
           
            #features_target = vgg(target)
            #features_outG   = vgg(input)
            #lossG_vgg       = F.mse_loss(features_target,features_outG)
            
            disc_output = modelD(output) 
            real_label = torch.ones(disc_output.shape).to(args.device)
            loss_adversarial = criterion(disc_output, real_label)
            #loss = F.l1_loss(output,target)
            
            losses.append(loss_adversarial)

            #break
        
        writer.add_scalar('Dev_Loss', sum(losses)/len(losses), epoch)
       
    return sum(losses)/len(losses), time.perf_counter() - start

def visualize(args, epoch, model_dc,data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            save_image(output, 'Reconstruction_unet')
            #save_image(output_dc, 'Reconstruction_dc_cnn')
            save_image(torch.abs(target.float() - output.float()), 'Error')
            break

def save_model(args, exp_dir, epoch, modelG_dc,optimizerG, modelD, optimizerD, best_dev_loss,is_new_best):

    out = torch.save(
        {
            'epoch': epoch,
            'args': args,
            'modelG_dc': modelG_dc.state_dict(),
...
def load_model(model_dc,checkpoint_file):
    
    checkpoint = torch.load(checkpoint_file)
    model_dc.load_state_dict(checkpoint['model'])
...
def main(args):
    ...
    if True:
        ...
        modelG_dc = build_model(args)
        modelG_dc = load_model(modelG_dc,args.checkpoint)
        modelD, optimizerD = build_discriminator(args)
        ...
        optimizerG = build_optim(args, modelG_dc.parameters())
        start_epoch = 0
        best_dev_loss = 1e9
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        train_lossG, train_lossD, train_time = train_epoch(args, epoch, modelG_dc,modelD, train_loader, optimizerG , optimizerD, writer, display_loader, args.exp_dir,vgg)

        print ("Epoch {}".format(epoch))
        print ("Validation for epoch :{}".format(epoch))
        dev_loss, dev_time = evaluate(args, epoch, modelG_dc, modelD, vgg, dev_loader, writer)
        
        print ("Visualization for epoch :{}".format(epoch))
        visualize(args, epoch, modelG_dc,display_loader, writer)

        is_new_best = dev_loss < best_dev_loss
        best_dev_loss = min(best_dev_loss,dev_loss)

        save_model(args, args.exp_dir, epoch, modelG_dc,optimizerG, modelD, optimizerD, best_dev_loss,is_new_best)
...
def create_arg_parser():
    ...
    parser.add_argument('--checkpoint', type=str,
                        help='path to dc-cnn best model')
