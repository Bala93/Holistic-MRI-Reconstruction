MODEL='dc-rsn-feat-redundancyremoved_UseAndAssistLatentDecoder-relu'
...
SEG_UNET_PATH=${BASE_PATH}'/experiments/'${DATASET_TYPE}'/'${MASK_TYPE}'/acc_5x/rsnlossfunctions/unetLEM/best_model.pt' # check this with sriprabha

#UNET_MODEL_PATH=${BASE_PATH}'/experiments/'${DATASET_TYPE}'/'${MASK_TYPE}'/acc_'${ACC_FACTOR}'/rsnlossfunctions/unetassisteverywhere-usereconfile/best_model.pt'

#DAUTOMAP_MODEL_PATH=${BASE_PATH}'/experiments/'${DATASET_TYPE}'/'${MASK_TYPE}'/acc_'${ACC_FACTOR}'/rsnlossfunctions/dASimpleRefinement-feat-usereconfile/best_model.pt'
...
#LEMAWARE_RSN_NC1_PATH=${BASE_PATH}'/experiments/'${DATASET_TYPE}'/'${MASK_TYPE}'/acc_'${ACC_FACTOR}'/rsnlossfunctions/lemawarersn_nc1_pretr_LemAssisteddASimpleUnetAssist_finetuneReNetworkwithrsn/best_model.pt'
...
#--lemaware_rsn_nc1_path ${LEMAWARE_RSN_NC1_PATH} 

#--unet_model_path ${UNET_MODEL_PATH} --dautomap_model_path ${DAUTOMAP_MODEL_PATH} 
#ACC_FACTOR_4x
...
LEMAWARE_RSN_NC1_PATH=${BASE_PATH}'/experiments/'${DATASET_TYPE}'/'${MASK_TYPE}'/acc_'${ACC_FACTOR}'/rsnlossfunctions/lemawarersn_nc1_pretr_LemAssisteddASimpleUnetAssist_finetuneReNetworkwithrsn/best_model.pt'
#python train.py --batch-size ${BATCH_SIZE} --num-epochs ${NUM_EPOCHS} --device ${DEVICE} --exp-dir ${EXP_DIR} --train-path ${TRAIN_PATH} --validation-path ${VALIDATION_PATH} --acceleration_factor ${ACC_FACTOR} --dataset_type ${DATASET_TYPE} --usmask_path ${USMASK_PATH} --unet_model_path ${UNET_MODEL_PATH} --dautomap_model_path ${dAUTOMAP_MODEL_PATH} --seg_unet_path ${SEG_UNET_PATH} --dncn_model_path ${DNCN_MODEL_PATH} --srcnnlike_model_path ${SRCNN_MODEL_PATH}
python train.py --batch-size ${BATCH_SIZE} --num-epochs ${NUM_EPOCHS} --device ${DEVICE} --exp-dir ${EXP_DIR} --train-path ${TRAIN_PATH} --validation-path ${VALIDATION_PATH} --acceleration_factor ${ACC_FACTOR} --dataset_type ${DATASET_TYPE} --usmask_path ${USMASK_PATH} --seg_unet_path ${SEG_UNET_PATH} --dncn_model_path ${DNCN_MODEL_PATH} --lemaware_rsn_nc1_path ${LEMAWARE_RSN_NC1_PATH} 
