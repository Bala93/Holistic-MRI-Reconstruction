...
class CSEUnetModelTakeLatentDecoder(nn.Module):
    """
    PyTorch implementation of a U-Net model.
    This is based on:
        Olaf Ronneberger, Philipp Fischer, and Thomas Brox. U-net: Convolutional networks
    ...
    def forward(self, input):
        ...
        for layer in self.up_sample_layers:
            ...
            outputup = F.upsample(output,size=(H,W),mode='bilinear')
            lf.append(outputup)
            upfeat.append(outputup)
        out = self.conv2(output)
        #lf.append(out)
        finalfeat=torch.cat(lf,dim=1)   
        finaldownfeat = torch.cat(downfeat,dim=1) 
        finalupfeat = torch.cat(upfeat,dim=1)
        #print("finalfeat shape: ", finalfeat.shape,"final downfeat shape: ", finaldownfeat.shape, "finalupfeat shape: ", finalupfeat.shape)  
        return finalfeat,out#finaldownfeat, finalupfeat, out
