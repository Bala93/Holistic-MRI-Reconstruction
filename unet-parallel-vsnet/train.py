...
def train_epoch(args, epoch, model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        output = model(input)
        output = F.pad(output,[2,2]) # pad based on axial
...
def visualize(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            input,target = data # Return kspace also we can ignore that for train and test
            input = input.to(args.device)[:,:,:,:-4]
            target = target.to(args.device)

            output = model(input.float())
            output = F.pad(output,[2,2]) # pad based on axial

            input  = complex_abs(input.permute(0,2,3,1)).unsqueeze(1)
            output = complex_abs(output.permute(0,2,3,1)).unsqueeze(1)
            target = complex_abs(target.permute(0,2,3,1)).unsqueeze(1)
