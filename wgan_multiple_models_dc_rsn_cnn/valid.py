...
def build_model(args):
    model = DnCnRefine(args,nc=5,mode='valid').to(args.device)
    return model
...
def run(args, model, data_loader):
    ...
    with torch.no_grad():
        for (input, kspace, target, fnames, slices) in tqdm(data_loader):
            input = input.float()
            input = input.unsqueeze(1).to(args.device)
            kspace = kspace.to(args.device)
            kspace = kspace.permute(0,3,1,2).float()
            recons = model(input, kspace).to('cpu').squeeze(1)
