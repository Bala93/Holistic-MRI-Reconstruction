...
from models import UnetModel, DataConsistencyLayer,dAUTOMAP,conv_block
import h5py
...
def load_dautomap(checkpoint_file):
    checkpoint = torch.load(checkpoint_file)
    args = checkpoint['args']
...
def run_unet(args, model, dautomap_model,unet_model,dc_layer,data_loader):
    ...
    with torch.no_grad():
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            #print("input_kspace size after permute: ",input_kspace.size())
...
def main(args):
    ...
    dautomap_model = load_dautomap(args.dautomap_model_path)
    unet_model     = load_unet(args.unet_model_path)
    ...
    reconstructions = run_unet(args, model, dautomap_model, unet_model, dc_layer, data_loader)
