...
class cnn_layer(nn.Module):
    ...
    def forward(self, xcrop,k,x):
        #print ('start',x.shape)
        
        x = x.permute(0,3,1,2)
        k = k.permute(0,3,1,2)
        xcrop = xcrop.permute(0,3,1,2)

        x = self.conv(xcrop,k,x)
        x = x.permute(0,2,3,1)
        return x
