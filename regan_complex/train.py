...
from models import CascadeUnet,Discriminator
...
def train_epoch(args, epoch, modelG, modelD, data_loader, optimizerG, optimizerD, writer, display_loader, exp_dir):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        fft_target = torch.fft(target.permute(0,2,3,1),2,True)
        fft_outG   = torch.fft(outG.permute(0,2,3,1),2,True)
        lossG_fft  = F.mse_loss(fft_target,fft_outG)

        lossG = img_scale * lossG_img + fft_scale * lossG_fft 
